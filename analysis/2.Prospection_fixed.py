...
# ============================================================
# FIXED VERSION of 2.Prospection.py
# Key bugs fixed:
#   1. [PRIMARY]   XPath apostrophe mismatch on "Date d'inscription au role:"
#                  The website renders a curled apostrophe (U+2019), not straight (U+0027).
#                  This caused a TimeoutException on EVERY address -> logged all as inaccessible.
#   2. [SECONDARY] Properties with no building (vacant lots) lack "Nombre de logements:"
#                  and "Annee de construction:" -> those are now optional with fallback to ''.
#   3. [MINOR]     Column name guard: supports both 'GOOGLE_MAPS' and 'GMAPS_URL' in Zonage.csv.
# ============================================================
...
def get_cell(xpath):
    """Waits for a required cell. Raises TimeoutException if not found within 5s."""
...
def optional_cell(xpath, timeout=3):
    """
    Returns the cell value, or '' if not found within timeout.
    Use for fields that may not exist (e.g. 'Nombre de logements' on vacant lots).
    """
    try:
...
for a in todo_addresses:
    ...
    try:
        ...
        # FIX #1: Use contains() to avoid apostrophe type mismatch.
        # BUG: '//tr[td[text()="Date d\'inscription au role:"]]/td[2]'
        # The site uses U+2019 curled apostrophe, not U+0027 straight.
        # text()= comparison was ALWAYS failing -> TimeoutException on every address.
        inscriptionDate = get_cell('//tr[td[contains(text(),"inscription au r")]]/td[2]')

        # FIX #2: optional_cell for building fields (vacant lots lack these rows).
        ...
        fields = [
            ...
            ownerName, ownerAddress, inscriptionDate, url, gmaps_url
        ]
